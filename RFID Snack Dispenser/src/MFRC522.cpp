...
{
    write_execute_bytes(reg, count, values);
} // End PCD_WriteRegister()
...
)
{
    ...
    while (index < count)
    {
        values[index] = read_execute(reg); // Read value and tell that we want to read the same address again.
        index++;
    }
    values[index] = read_execute(reg); // Read the final byte. Send 0 to stop reading.
} // End PCD_ReadRegister()
